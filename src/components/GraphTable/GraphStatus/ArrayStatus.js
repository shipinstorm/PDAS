...
export default function ArrayStatus({
  ...
  const setStatusPercents = () => {
    ...
    if (total > 0){
      for (var status of statuses){
        status.percent = parseInt(selectedArrayData[status.mapping])/total*100;
    ...
    <>
      ...
      >
        ...
        <span className="text-overflow-center">
          {numDone>0 &&
          <span className="status-done">{numDone} done</span>}
          {numRun>0 &&
      ...
      <div className={"row status-" + statusClass}>
        <div className="status-text text-center">
          ...
          <span
            ...
            className={(!isSpeculative||status!='running') ? "not-speculative" : ""}
          >
            (S)
          </span>
        </div>
      ...
      <div className="row">
        ...
        <div className="status-bar-div"><img src="images/spinner_dark.gif" alt='' /></div>
      </div>}
    </>
  )
}